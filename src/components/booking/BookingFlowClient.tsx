      ...
      <div className="max-w-3xl mx-auto px-4">
        {providerId && (
          <p className="text-center text-sm text-slate-500 mb-4 font-medium">
            Booking with provider <span className="text-fuchsia-600 font-bold">{providerId}</span>
          </p>
        )}
                ...
                <div className="grid grid-cols-3 gap-3">
                  {['09:00', '12:00', '15:00'].map((t) => (
                    <button
                      key={t}
                      type="button"
                      className="py-4 bg-white border-2 border-slate-100 rounded-2xl font-black hover:border-fuchsia-500 hover:text-fuchsia-600 transition-all"
                    >
                      {t}
                    </button>
                  ))}
            ...
            {currentStep === 2 && (
              <div className="space-y-6">
                ...
                <p className="text-slate-600 font-medium">
                  Review your selection before confirming. You can still go back to adjust the service or time window.
                </p>
                <ul className="space-y-3 text-slate-800 font-semibold border-2 border-slate-100 rounded-2xl p-6 bg-slate-50/40">
                  <li>Service: Emergency Repair (example)</li>
                  <li>Schedule: selected date & slot</li>
                  {providerId && <li>Provider ID: {providerId}</li>}
                </ul>
              </div>
            )}
